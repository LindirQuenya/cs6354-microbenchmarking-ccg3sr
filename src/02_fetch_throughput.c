...
// In optimized form, each loop has 10 instructions.
 __attribute__((optimize("no-unroll-loops"))) calibrated_stats fetch_throughput8(int iterations) {
	long long start, end;
...
// In optimized form, each loop has 18 instructions.
 __attribute__((optimize("no-unroll-loops"))) calibrated_stats fetch_throughput16(int iterations) {
	long long start, end;
	unsigned int tsc_aux;
	int *times = malloc(sizeof(int) * iterations);
	int *times_calib = malloc(sizeof(int) * iterations);
