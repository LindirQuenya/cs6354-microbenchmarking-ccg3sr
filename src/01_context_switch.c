...
calibrated_stats contextswitch_thread(int iterations) {
	...
	// For holding differences. The differences will be much smaller than INTMAX.
	int *times = malloc(sizeof(int) * n_test);
	// This one is for the calibration run. It doesn't need explicit start and end arrays;
	// it will only be running on one thread.
...
void * contextswitch_ping(void * ptr) {
	...
	for (long i = 0; i < n_test; i++) {
		// Synchronize up with the other thread to have a consistent start point.
...
void contextswitch_pingpong(int *store) {
	...
	for (long i = 0; i < n_test; i++) {
		...
		sem_post(&sem);
		// We are incurring a function call overhead here. This is fine; the real thing
		// has a usleep() function call to match. And this way we get to sample the 
		// consequences of performing sem_wait's cleanup.
		sem_wait(&sem);
